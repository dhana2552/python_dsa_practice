...
class LinkedList:
    ...
    def selection_sort(self):
        if self.length < 2:
            return
        i = self.head
        while i.next: #range(len(my_list)-1)
            min_index = i
            j = i.next
            while j: #range(i+1, len(my_list))
                if min_index.value > j.value: #compare the values
                    min_index = j
                j = j.next #move to the next pointer
            if i != min_index:
                i.value, min_index.value = min_index.value, i.value #swap the values
            i = i.next #move to the next pointer
